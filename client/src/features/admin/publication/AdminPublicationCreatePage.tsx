...
export default function AdminPublicationCreatePage() {
    ...
    // указываем, что для текущего типа нужна цитата
    // для афоризма не нужен заголовк
    // Для всех других типов (новость, эссе, статья, отзыв) обязательно наличие заголовка и основного текста
    ...
    async function handleSubmit(event: React.FormEvent) {
        ...
        try {
            ...
            const payload = {
                ...
                titleEn: normalizeInput(titleEn),
                slug: normalizeInput(slug),
                body: normalizeInput(body),
                bodyEn: normalizeInput(bodyEn),
                quoteText: normalizeInput(quoteText),
                quoteTextEn: normalizeInput(quoteTextEn),
                isPublished,
                publishedAt: normalizeInput(publishedAt),
            };

            // маршрут post для создания публикации
            await api.post("/api/admin/publications", payload);
            // наигация на старницу публикации
            navigate("/admin/publications");
        } catch (submitError) {
            const err = submitError as AxiosError<{ message?: string }>;
            setError(err?.response?.data?.message || "Ошибка создания публикации");
        } finally {

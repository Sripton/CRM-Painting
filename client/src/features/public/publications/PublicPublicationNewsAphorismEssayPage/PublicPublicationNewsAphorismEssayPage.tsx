...
type VisibleSection = "NEWS" | "APHORISM" | "ESSAY";
...
export default function PublicPublicationNewsAphorismEssayPage() {
    ...
    const [activeSection, setActiveSection] = useState<'NEWS' | 'APHORISM' | 'ESSAY'>('NEWS');
    ...
    useEffect(() => {
        async function loadPublications() {
            try {
                ...
                console.error("Не удалось загрузить публикации NEWS/APHORISM/ESSAY", error as AxiosError);
            }
        }
        loadPublications();
    }, []);
    ...
    const hasLongText = (publication: PublicationFull) => {
        ...
        return values.some((value) => (value?.length ?? 0) > textPreviewLimit);
    };
